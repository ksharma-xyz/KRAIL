...
| 2026-09-23 | `app_start` | `aiCapability`, `aiSearchEnabled` | `aiCapability`: `available｜model_downloading｜device_unsupported｜needs_device_setting｜check_failed｜timeout｜unknown`. `aiSearchEnabled`: Bool, omitted if the flag could not be read | Every launch, **whatever the flag says**: the denominator for `ask_krail_attempt`. `needs_device_setting` is a rider who switched on-device AI off (iOS only). The check never starts a model download. `app_start` now fires once the check answers or after 2 s, whichever is first, instead of immediately; the splash does not wait for it | (local) | Pending | `docs/plans/ASK_KRAIL_ANALYTICS_PLAN.md` |
