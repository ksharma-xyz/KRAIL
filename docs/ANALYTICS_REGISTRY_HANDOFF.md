...
| 2026-07-22 | `park_ride_user_facility` | NEW event: `facilityId`, `stopId`, `action`, `source` | `action` = `add｜remove`; `source` = `picker｜home` (only `picker` emitted so far) | Rider adds or removes a Park & Ride facility they picked themselves. Add/remove share one name and split on `action` rather than spending two slots; `source` reserved for a second entry point | [#1740](https://github.com/ksharma-xyz/KRAIL/issues/1740) | Pending | — |
