...
internal class RealGtfsRealtimeService(
    ...
    override suspend fun fetchFeed(
        ...
        val url = buildUrl(baseUrl = baseUrl, feedName = feedName, feedType = feedType)
        ...
        logNetworkCall(
            target = baseUrl.toNetworkUpstream(),
            method = "GET",
            path = url.removePrefix(baseUrl),
        )
        ...
        log("GtfsRT: GET $url status=$statusCode, Content-Type=$contentType, Last-Modified=$lastModified")
    ...
    private fun buildUrl(baseUrl: String, feedName: String, feedType: GtfsFeedType): String =
        buildGtfsUrl(baseUrl = baseUrl, feedName = feedName, feedType = feedType)
