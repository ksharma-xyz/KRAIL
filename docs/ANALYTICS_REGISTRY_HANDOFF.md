...
| 2026-08-09 | `alert_summary_status` (NEW EVENT) | `alertId`, `action` | `alertId` = hash of alert heading+message (no server-provided alert id exists); `action` = `shown｜upvote｜downvote` | On-device AI summary lifecycle on `CollapsibleAlert`, gated behind the `alert_summary_enabled` remote-config kill switch. No summary/alert text in params by design | (local) | Pending | — |
